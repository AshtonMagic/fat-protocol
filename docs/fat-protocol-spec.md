...
discussions-to: https://ethereum-magicians.org/t/erc-8409-tokenized-agent-protocol
status: Draft
...
- **Share** — a transferable unit of equity issued by the Agent, representing participation in its economic performance, implemented as ERC-20.
...
2. The Accept Token is a single ERC-20; the Agent MUST NOT accept native ether as payment for Shares (§6.3.1).
...
6. Request cancellation and request expiry / TTL are OPTIONAL and implementer-defined (§7). A cancelling or expiring Agent MUST return still-pending escrowed Shares (redeem) or still-pending deposited Accept Token (mint) to the requester; already-settled (claimable) balances are unaffected. Such an Agent SHOULD define its own cancellation event.
...
2. Read-only reentrancy into view functions (`isExecutor`, `queryMintStatus`, `queryRedeemStatus`, `exchangeRate`, ERC-20 `balanceOf`) is not forbidden; implementers SHOULD ensure any values exposed by views are consistent with the Agent's post-transaction invariants.
...
- Whether Shares are transferable. A compliant Agent MAY override ERC-20 transfers to restrict, pause, or tax them.
...
- **ERC-165** — implementers MUST advertise FAT support via ERC-165 (`supportsInterface`), using the XOR of the function selectors in §4, so that indexers, wallets, and composing contracts get a protocol-level discoverability guarantee. The canonical interface ID will be fixed when this spec leaves Draft.
...
**Stranded pending requests.** While balances are pending, a requester's deposited Accept Token (mint) sits in the Agent and their escrowed Shares (redeem) are held but not yet burned. If settlement never triggers — the Owner abandons the Agent, the Executor key is lost, or the contract is bricked — those balances can be stranded with no claim path, and for a pending mint the requester is not even a Holder. Agents SHOULD either provide a path for a Requester to reclaim a still-pending deposit/escrow, or disclose in the Agent URI metadata that requests are irreversible once submitted, together with the bounds of that irreversibility (e.g., a maximum settlement deadline). (Whether to make such a reclaim-or-disclose rule a conformance requirement is an open design question for the standard's editors.)
